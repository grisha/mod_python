...
__ANSWER = 42
URI_FILE = os.path.split(__file__)[-1]
...
class SimpleTestCase(unittest.TestCase):
    ...
    def test_req_members(self):
        ...
        if req.unparsed_uri != "/%s" % (URI_FILE):
            self.fail("req.unparsed_uri should be '/%s'" % URI_FILE)
            
        log("    req.uri: %s" % `req.uri`)
        if req.uri != "/%s" % (URI_FILE):
            self.fail("req.uri should be '/%s'" % URI_FILE)
        ...
        if req.parsed_uri[apache.URI_PATH] != '/%s' % (URI_FILE):
            self.fail("req.parsed_uri[apache.URI_PATH] should be '/%s'" % (URI_FILE))
