...
user. This simple application consists of two files: \filenq{form.html},
the form to collect the data, and \filenq{form.py}, the target of the
form's action.
...
program, since it has access to the \class{Request} object. You can
do all the same things you can do with a "native" mod_python handler,
e.g. set custom headers via \code{req.headers_out}, return errors by
raising \exception{apache.SERVER_ERROR} exceptions, write or read
directly to and from the client via \method{req.write} and
\method{req.read}, etc.
...
mod_python, you need to understand what a handler is. And it's really
rather simple.
...
Note that a handler must take one argument - the \class{Request}
object. The \class{Request} object is an object that provides all of
...
client is also done via the \class{Request} object, i.e. there is no
"response" object.
...
\constant{apache.OK}. Apache will then proceed to the next
handler. (which in this case would be \function{handler()} if it's a
\code{.py} file).
