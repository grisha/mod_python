...
functions, all documented below. (The \class{Request} object also
...
If you have a reference to a \class{Request} object available,
consider using \method{Request.log_error} intead, it will prepend
...
This function is deprecated and is a reference to \class{table}.
\end{funcdesc}
...
object performs very much like a dictionary, with the only difference
that key lookups are case insensitive and both keys and values must be
strings.
...
\end{methoddesc}
% THE FUN STARTS HERE - MAKE REQUEST a NEW object. GT HERE.
...
Adds methods to the \member{Request.allowed_methods} list. This list
will be passed in \code{Allowed:} header if \constant{HTTP_METHOD_NOT_ALLOWED}
...
\constant{METHOD_NOT_ALLOWED}. This member can be modified via \method{Request.allow_methods} 
...
You may have to call \member{Request.add_common_vars} first to fill in the information
you need.
\end{memberdesc}
...
data between handlers, it's better to simply add members to the \class{Request}
object than to use \member{notes}.
\end{memberdesc}

\begin{memberdesc}[Request]{content_type}
String. The content type. Mod_python maintains an internal flag
(\member{Request._content_type_set}) to keep track of whether
...
by the URI.
\code{(valid, protection, user, group, inode, device, nlink, size, csize, atime, mtime, ctime, fname, name)}. The \code{apache} module defines a set of \constant{FINFO_*} constants that
should be used to access elements of this tuple. Example:
\begin{verbatim}
