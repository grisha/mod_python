...
Multiple handlers can be specified on a sinlge line, in which case
...
result - all hanlders listed will be executed sequentially, from first
...
of type <class>, it will try instantiate it passing it a single
...
them. Terefore, \code{req.register_cleanup()} has no effect within this
handler.
...
PythonInterpreter have no effect on the behaviour of modules imported
...
global intepreter. This may not be exactly what you want, but
...
existance of functions matching the default handler function names,
