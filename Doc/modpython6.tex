...
The object passed in \code{psp} variable at this point supports only
one function, \code{set_error_page(string)} which can be used to set
(a relative path to) a psp page to be process when an exception
occurs. This page will receive one additional variable,
\code{exception}, which is a 3-tuple returned by \code{sys.exc_info()}.
